...
// use crate::client::ret_failed_resp;
use crate::client::{
    error::{Error, Result},
    ipc::{IpcConnectionPool, LocalSocket},
    models::{
        BaseConfig, CloseFrame, ConnectionId, ConnectionManager, Connections, CoreUpdaterChannel,
        ErrorResponse, Groups, LogLevel, MihomoVersion, Protocol, Proxies, Proxy, ProxyDelay,
        ProxyProvider, ProxyProviders, RuleProviders, Rules, WebSocketMessage, WebSocketWriter,
    },
    utils,
};
...
impl Mihomo {
    ...
    pub fn update_socket_path<S: Into<String>>(&mut self, socket_path: S) -> Result<()> {
        ...
        // the same with ipc.rs: 569
        tokio::runtime::Handle::current().block_on(pool.clear_pool());
        Ok(())
    }
        ...
        F: Fn(serde_json::Value) + Send + 'static,
    {
        ...
        let handle_message = |message| {
            ...
            match message {
                ...
                Ok(Message::Close(t)) => {
                    serialize_with_fallback(WebSocketMessage::Close(t.map(|v| CloseFrame {
                        code: v.code.into(),
        ...
        match self.protocol {
            ...
            Protocol::LocalSocket => {
                if let Some(socket_path) = self.socket_path.as_ref() {
                    ...
                    let stream = crate::client::ipc::connect_to_socket(socket_path).await?;
    ...
    /// Send message to specified WebSocket connection (temporarily no place to use)
    ...
    /// WebSocket: Mihomo memory usage data
    ...
    /// WebSocket: Mihomo connections information data
        ...
        F: Fn(serde_json::Value) + Send + 'static,
    {
        ...
        let ws_url = match self.protocol {
            // format of parameter (format=structured) added behind url is:
    ...
    /// Close specified connection by ID
    ...
    /// Get specified proxy group by name
    ...
    /// Perform delay test for specified proxy group, and cleanup it's fixed node.
    ...
    /// Get proxy providers information
    ...
    /// Get specified provider information by name
    pub async fn get_proxy_provider_by_name(&self, provider_name: &str) -> Result<ProxyProvider> {
        ...
        let client =
            self.build_request(Method::GET, &format!("/providers/proxies/{provider_name}"))?;
        let response = self.send_by_protocol(client).await?;
        if !response.status().is_success() {
            let err_msg = response.json::<ErrorResponse>().await.map_or_else(
    ...
    /// Update specified proxy provider information
    pub async fn update_proxy_provider(&self, provider_name: &str) -> Result<()> {
        let provider_name = urlencoding::encode(provider_name);
        let client =
            self.build_request(Method::PUT, &format!("/providers/proxies/{provider_name}"))?;
        let response = self.send_by_protocol(client).await?;
        if !response.status().is_success() {
            let err_msg = response.json::<ErrorResponse>().await.map_or_else(
    ...
    /// Perform healthcheck to specified proxy provider
    ...
    /// Perform healthcheck to specified node (not proxy group) of specified proxy provider,
    /// and return new delay information.
    ...
    /// Get specified proxy information
    ...
    /// Choose node for specified proxy
    ///
    /// Generally used to specify a proxy node under a specified proxy group [proxy group/node]
    ...
    /// Specifies that fixed proxy nodes will no longer be used under the specified proxy group
    ///
    /// Generally used for nodes under automatically selected proxy groups (e.g., proxy groups of type URLTest).
    ...
    /// Perform delay tests on the specified proxy
    ///
    /// Generally used for delay testing of proxy nodes,
    /// it can also pass the proxy group name
    /// (only the selected proxy nodes under the proxy group will be tested)
    ...
    /// Get all rule providers information
    ...
    /// Update rule provider information
    ...
    pub async fn patch_base_config<D: serde::Serialize + Clone + Sync>(
        &self,
        data: &D,
    ) -> Result<()> {
    ...
    /// Update Geo, same with [`upgrade_geo`](crate::client::mihomo::Mihomo::upgrade_geo)
