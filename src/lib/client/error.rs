...
use crate::client::models::ConnectionId;
...
impl From<tokio_tungstenite::tungstenite::Error> for Error {
    fn from(e: tokio_tungstenite::tungstenite::Error) -> Self {
        crate::client::error::Error::Websocket(e.to_string())
    }
}

impl From<std::string::FromUtf8Error> for Error {
    fn from(e: std::string::FromUtf8Error) -> Self {
        crate::client::error::Error::ParseError(e.to_string())
    }
}
...
macro_rules! failed_resp {
    ($($arg: tt)*) => {
        $crate::client::error::Error::FailedResponse(format!($($arg)*))
    };
}
...
macro_rules! ret_failed_resp {
    ($($arg: tt)*) => {
        // return Err($crate::client::failed_resp!($($arg)*))
        return Err(failed_resp!($($arg)*))
    };
}
